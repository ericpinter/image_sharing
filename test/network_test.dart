...
void main() {
  ...
  test('Image Translation', () async {
    var w_img = Widget.Image.asset("images/test.png");
    var u_img = await WidgetImagetoUI(w_img);
    expect(u_img, isNotNull);
    var new_w_img = await WidgetImagetoUI(w_img);
    expect(new_w_img, isNotNull);
  });
  ...
  test('Transfer image', () async {
    ...
    var log = await NetworkLog.getLog(() {image_received.complete();});
    ...
    expect(log.feed[0].image.height, Widget.Image.memory(await picked.readAsBytes()).height); //actually testing the images being the same is hard, but this is close
